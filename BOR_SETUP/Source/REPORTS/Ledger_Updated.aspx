<%@ Page Title="" Language="C#" MasterPageFile="~/MasterPage/Master.master" AutoEventWireup="true" CodeFile="Ledger_Updated.aspx.cs" Inherits="REPORTS_Ledger_Updated" %>
...
<div class="col-md-10"><asp:DropDownList ID="ddlITEM" runat="server" DataSourceID="SqlDataSource1" DataTextField="AccountsTitle" DataValueField="AccountsID"  CssClass="chzn-select form-control" ></asp:DropDownList></div>
<div class="col-md-2"><asp:Button CssClass="btn btn-info" runat="server" id="btnSave" OnClick="LoadReport" Text="VIEW REPORT" Width="100%" Font-Bold="true"  OnClientClick="document.forms[0].target = '_blank';"   /></div>
                            ...
                            <asp:SqlDataSource ID="SqlDataSource1" runat="server" ConnectionString="<%$ ConnectionStrings:Con %>" 
                                            SelectCommand="SELECT '0' as [AccountsID],' -- SELECT ALL Accounts -- ' as [AccountsTitle] FROM [Accounts] UNION  SELECT   [AccountsID], [AccountsTitle] FROM [Accounts] where BranchID=@BranchID Order by AccountsTitle">
                                            <SelectParameters>
