<%@ Page Title="" Language="C#" MasterPageFile="~/MasterPage/Master.master" AutoEventWireup="true" CodeFile="PO_Local_Edit.aspx.cs" Inherits="PROCUREMENT_PO" %>
...
<!-- #modal-message -->
  ...
  <div class="row">
			    ...
			    <div class="col-md-12">
                    ...
                    <h3 class="panel-title" style="color:White;"> <span class="menu-icon"> <i class="icon-pie"></i> </span> Purchase Order ( LOCAL ) EDIT</h3>
                    <div class="vd_panel-menu">
  <div data-action="minimize" data-original-title="Minimize" data-toggle="tooltip" data-placement="bottom" class=" menu entypo-icon"> <i style="color:White;" class="icon-minus3"></i> </div>
  <div data-action="refresh"  data-original-title="Refresh" data-toggle="tooltip" data-placement="bottom" class=" menu entypo-icon smaller-font"> <i style="color:White;" class="icon-cycle"></i> </div>
                        ...
                        <div class="panel-body">
                         ...
                         <div class="row"><div class="col-lg-12" style="height:10px"></div></div>
                            ...
                            <div class="row" style="overflow:visible;">
                                <div class="col-lg-12 table-responsive" style="overflow:visible;" id="NewDiv">
                            ...
                            <%-- <input type="button" id="btnClear" class="btn btn-danger btn-sm" value="Clear Form" onclick="ClearForm();" />--%>
                             <input type="button" id="btnSave" style="font-weight:bold; font-size:20px;" class="btn btn-warning btn-block" value="U P D A T E &nbsp&nbsp&nbsp P U R C H A S E  &nbsp&nbsp&nbsp O R D E R" onclick="SaveTransaction();" />
                             </div></div>
...
<script type="text/javascript">
    ...
    function GetRMB() {
        ...
        $.ajax({
            type: "POST",
            url: "PO_Local_Edit.aspx/LoadRMbValue",
            data: dataToSend,            
            dataType: "json",
    ...
    function LOAD_ITEMS() {
        ...
        $.ajax({
            type: 'POST',
            url: 'PO_Local_Edit.aspx/LOAD_ITEMS',
    ...
    function LoadPurchaseData() {
        ...
        var tblRows = "";
        var UserID = "1";
        var tbl = "<table id='tblItems' class='table table-striped table-bordered dataTable no-footer' style='overflow:visible;'><tbody><tr><th style='width:5%'>Sr1.</th><th style='width:35%'>Item</th><th style='width:7%'>Qty.</th><th style='width:12%'>Unit Price (PKR)</th><th style='width:12%'>Unit Price (RMB)</th><th style='width:12%'>Total Price (PKR)</th><th style='width:12%'>Total Price (RMB)</th><th style='width:5%'></th></tr>";
        var cc = 0;
        var ccValue = 0;
        $.ajax({
            type: 'POST',
            url: 'PO_Local_Edit.aspx/LOAD_PURCHASE',
    ...
    function tot(a) {
        ...
        if (thisID.indexOf('txtQty') != -1) {
            ...
            if (document.getElementById("ddlPurchaseType").value == "RMB") {
                ...
                BalanceID = thisID.replace('txtRMBPrice', 'txtBalance');
                OrgQtyID = thisID.replace('txtRMBPrice', 'txtOriginalQty');
            }
        }
        ...
        if (parseFloat(Qty) < parseFloat(Balance)) {
            ...
            alert("Can not reduce Quantity... !! Quantity Sold.");
            return;
        }
...
//        }
        ...
        for (var i = 0; i < elms.length; i++) {
            if (elms[i].id.indexOf('txtAccCode') != -1) {
                ...
                var txtBatchID = iD.replace('txtAccCode', 'txtDetailID');
                var txtBalanceID = iD.replace('txtAccCode', 'txtBalance');
                ...
                var txtBatch = document.getElementById(txtBatchID).value;
                var txtBalance = document.getElementById(txtBalanceID).value;                
                ...
                if (txtQty != 0) {
                    if (cc == 0) {
                        str = txtAccID + "^" + txtAccTitle + "^" + txtQty + "^" + txtPrice + "^" + txtTotalPrice + "^" + txtPriceRMB + "^" + txtTotalPriceRMB + "^" + txtRMBValue + "^" + txtSrNo + "^" + txtBatch + "^" + txtBalance;
                    }
                    else {
                        str = str + "`" + txtAccID + "^" + txtAccTitle + "^" + txtQty + "^" + txtPrice + "^" + txtTotalPrice + "^" + txtPriceRMB + "^" + txtTotalPriceRMB + "^" + txtRMBValue + "^" + txtSrNo + "^" + txtBatch + "^" + txtBalance;
                    }
                }
        ...
        if (cc > 0) {
            $.ajax({
                type: 'POST',
                url: 'PO_Local_Edit.aspx/SaveTransaction',
                //data: {},
                ...
                success: function (msg) {
                    tblRows = msg.d;
                    alert('PO has been Updated Successfully!');
    ...
    <script type="text/javascript">
        ...
        $(document).on('keydown', function (e) {
            ...
            if (e.ctrlKey && String.fromCharCode(kc).toUpperCase() == "A") {
                e.preventDefault();
                //    addRow();
                alert("You can not add row in Purchase Order Edit Form...!");
            }
        });
